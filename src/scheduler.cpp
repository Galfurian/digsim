...
void scheduler_t::schedule_now(const process_info_t &proc_info)
{
    ...
    simcore::trace("scheduler_t", "[#queue = {:-2}] Now: {} (now)", event_queue.size(), proc_info.to_string());
}
...
void scheduler_t::initialize()
{
    ...
    simcore::trace("scheduler_t", "[#queue = {:-2}] -- Check for bad cycles", event_queue.size());
    // First, compute the cycles in the dependency graph.
    simcore::dependency_graph.compute_cycles();
    // Check the cycles.
    for (const auto &cycle : simcore::dependency_graph.get_cycles()) {
        if (simcore::dependency_graph.is_bad_cycle(cycle)) {
            simcore::error("scheduler_t", "Bad cycle detected:");
            simcore::dependency_graph.print_cycle_report(cycle);
            simcore::error("scheduler_t", "Exporting DOT graph as `bad_cycle_graph.dot`.");
            simcore::dependency_graph.export_dot("bad_cycle_graph.dot");
            simcore::error("scheduler_t", "Exiting.");
            std::exit(EXIT_FAILURE);
        }
    }
    // Run all initialization callbacks.
    if (!initializer_queue.empty()) {
        simcore::trace("scheduler_t", "[#queue = {:-2}] -- Begin initialization cylce", event_queue.size());
...
void scheduler_t::run(discrete_time_t simulation_time)
{
    ...
    while (!event_queue.empty()) {
        ...
        simcore::trace("scheduler_t", "[#queue = {:-2}] -- Begin cylce", event_queue.size());
        ...
        // Now run the batch.
        if (!batch.empty()) {
            simcore::trace("scheduler_t", "[#queue = {:-2}] -- Run batch", event_queue.size());
...
void scheduler_t::print_event_queue() const
{
    ...
    if (!time_buckets.empty()) {
        simcore::trace("scheduler_t", "[#queue = {:-2}] -- Event queue", event_queue.size());
        for (const auto &[t, names] : time_buckets) {
            ...
            ss << "]";
            simcore::trace("scheduler_t", "[#queue = {:-2}] {}", event_queue.size(), ss.str());
        }
    } else {
        simcore::trace("scheduler_t", "[#queue = {:-2}] -- Event queue is empty", event_queue.size());
    }
}
