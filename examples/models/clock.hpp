...
class Clock : public simcore::module_t
{
public:
    ...
    Clock(
        ...
        simcore::discrete_time_t clk_period     = 2,
        double clk_duty_cycle                  = 0.5,
        simcore::discrete_time_t clk_start_time = 0,
        bool clk_posedge_first                 = false)
        : simcore::module_t(clk_name)
        , out("out", this)
        , period(clk_period)
        , duty_cycle(clk_duty_cycle)
    {
        ...
        if (clk_posedge_first) {
            delay += static_cast<simcore::discrete_time_t>(static_cast<double>(period) * duty_cycle);
        } else {
            delay += static_cast<simcore::discrete_time_t>(static_cast<double>(period) * (1 - duty_cycle));
        }
        // Create a process info for the clock evaluation method.
        auto proc_info = simcore::get_or_create_process(this, &Clock::evaluate, "start");
        // Schedule the first evaluation of the clock signal.
        simcore::scheduler.schedule_after(proc_info, delay);
...
private:
    ...
    void evaluate()
    {
        ...
        auto proc_info = simcore::get_or_create_process(this, &Clock::evaluate, "evaluate");
        // Schedule the next evaluation of the clock signal.
        simcore::scheduler.schedule_after(proc_info, delay);
    }

    /// @brief Period of the clock signal in discrete time units.
    simcore::discrete_time_t period;
